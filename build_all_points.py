...
<body>
  ...
  <script>
    ...
    function popupHtmlListing(item) {
      ...
      return `<b>${item.title || 'Listing'}</b>${location}${price}${size}${rooms}${portal}${propertyType}${category}${recent}${matched}${linkLine('Open listing', item.listing_url)}${linkLine('Open source search', item.source_url)}`;
    }
